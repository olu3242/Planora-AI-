...
import { profileWorkbook, selectPrimarySheet } from "@/integrations/spreadsheets/workbook-profiler";
...
export async function ingestWorkbook(file: File, context: ActorContext) {
  ...
  const primary = selectPrimarySheet(parsed, profile);
...
export async function validateAndImportWorkbook(input: ActorContext & { workbookId: string }) {
  ...
  const profileData = workspace.profile?.profile as unknown as import("@/integrations/spreadsheets/types").WorkbookProfileResult;
  const sheet = selectPrimarySheet(parsed, profileData);
  if (!sheet) throw new AppError("IMPORT_ERROR", "Workbook has no importable P&L data block.", 422);
