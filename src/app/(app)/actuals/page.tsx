...
export default async function ActualsPage({ searchParams }: { searchParams: Search }) {
  ...
  const explanation = ebitda.metricValueId ? await explainMetricValue(session.organization.id, ebitda.metricValueId, statement.selected) : null;
  const lineageQuery = new URLSearchParams(); if (statement.selected.geographyId) lineageQuery.set("geography", statement.selected.geographyId); if (statement.selected.productId) lineageQuery.set("product", statement.selected.productId);
  return <>
